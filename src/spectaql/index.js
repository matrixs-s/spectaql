...
async function run(opts) {
  ...
  if (extensions?.addManualResponseForJsonType && extensions?.manualResponseJsonFile) {
    let apiResponseJson = {};
    try {
      apiResponseJson = JSON.parse(fs.readFileSync(extensions?.manualResponseJsonFile, 'utf8'));
    } catch (error) {
      console.error('Error constructing api response: ', error.stack)
    }
    extensions.apiResponseJson = apiResponseJson;
  }
